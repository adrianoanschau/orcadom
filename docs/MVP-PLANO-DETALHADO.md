...
- [x] `pnpm init` na raiz.
- [x] Criar `pnpm-workspace.yaml` apontando para `apps/*` e `packages/*`.
- [x] Criar `turbo.json` com pipelines básicos (`build`, `dev`, `lint`, `test`).
- [x] Criar `docker-compose.yml` com o serviço `postgres`.
- [x] Criar `.env` a partir de um `.env.example` versionado.
- [x] Rodar `docker compose up -d` e confirmar que o Postgres aceita conexões.
- [x] Criar `packages/config` com `tsconfig.base.json` e config de ESLint
  ```
  compartilhada.
  ```
...
- [x] Criar `packages/database` com `package.json` próprio.
- [x] Instalar Prisma (`prisma` como dev dependency, `@prisma/client` como
  ```
  dependency).
  ```
- [x] Escrever `schema.prisma` com os models `User`, `Account`, `Category`
  ```
  e `Transaction` (ver `02-modelagem-dados.md` para o schema completo).
  ```
- [x] Rodar `npx prisma migrate dev --name init` e confirmar a criação das
  ```
  tabelas.
  ```
- [x] Criar `src/index.ts` no pacote exportando um `PrismaClient` singleton
  ```
  (evita múltiplas conexões em ambiente de desenvolvimento com hot reload).
  ```
- [x] Criar `packages/types` e definir o primeiro schema Zod (ex:
  ```
  `createTransactionSchema`), validando que o build entre pacotes do
  monorepo funciona (`turbo run build` sem erro).
  ```
- [x] (Opcional, recomendado) Criar um script de seed
  ```
